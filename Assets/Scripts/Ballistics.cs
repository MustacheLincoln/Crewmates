...
public static class Ballistics
{
    ...
    public static bool CheckBallisticPath(Vector3[] arc, LayerMask lm)
    {
        ...
        for (int i = 1; i < arc.Length; i++)
        {
            ...
            if (Physics.Raycast(arc[i - 1], arc[i] - arc[i - 1], out hit, (arc[i] - arc[i - 1]).magnitude))// && GameMaster.IsInLayerMask(hit.transform.gameObject.layer, lm)) <============NOT SURE WHAT THIS IS BUT BEN HAD TO REMOVE IT
                return false;
